...
import { SlidersHorizontal, Globe, ChevronLeft, ChevronRight, Users } from 'lucide-react';
...
const ChatHub: React.FC<{
  ...
  return (
    <div className="flex h-full min-h-0 flex-col xl:flex-row">
      ...
      {/* Right — browser agent sandbox */}
      {settings.sandboxOpen && (
        <aside className="flex h-[46vh] shrink-0 flex-col border-t border-white/8 bg-[#0F1016] xl:h-auto xl:w-[420px] xl:border-l xl:border-t-0">
          <div className="flex shrink-0 items-center justify-between border-b border-white/8 px-3 py-2">
            <span className="flex items-center gap-1.5 text-[10.5px] font-semibold text-white/55">
              <Globe className="h-3.5 w-3.5" /> Browser agent sandbox
            </span>
            <button
