              ...
              <div className="rounded-2xl border border-orange-500/30 bg-[#161724]/75 p-5 space-y-4 shadow-xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3 items-center rounded-xl bg-black/40 border border-white/10 p-3.5">
                  <div className="md:col-span-2 space-y-1">
                    <label className="text-[10.5px] font-mono uppercase tracking-wider text-orange-300 font-semibold flex items-center justify-between">
                      <span>Select Major Sports Team:</span>
                      <span className="text-white/40">({visibleSportsTeams.length} franchises)</span>
                    </label>
                    <div className="relative">
                      ...
                      >
                        {selectedSport === 'all' ? (
                          <>
                            <optgroup label="🏈 Football (NFL)">
                              {SPORTS_TEAMS_CATALOG.filter((t) => t.sport === 'football').map((team) => (
                                <option key={team.id} value={team.id}>
                                  {team.name} — {team.city} (NFL)
                                </option>
                              ))}
                            </optgroup>
                            <optgroup label="⚾ Baseball (MLB)">
                              {SPORTS_TEAMS_CATALOG.filter((t) => t.sport === 'baseball').map((team) => (
                                <option key={team.id} value={team.id}>
                                  {team.name} — {team.city} (MLB)
                                </option>
                              ))}
                            </optgroup>
                            <optgroup label="🏀 Basketball (NBA)">
                              {SPORTS_TEAMS_CATALOG.filter((t) => t.sport === 'basketball').map((team) => (
                                <option key={team.id} value={team.id}>
                                  {team.name} — {team.city} (NBA)
                                </option>
                              ))}
                            </optgroup>
                            <optgroup label="🏒 Hockey (NHL)">
                              {SPORTS_TEAMS_CATALOG.filter((t) => t.sport === 'hockey').map((team) => (
                                <option key={team.id} value={team.id}>
                                  {team.name} — {team.city} (NHL)
                                </option>
                              ))}
                            </optgroup>
                            <optgroup label="⚽ Soccer (MLS / Global)">
                              {SPORTS_TEAMS_CATALOG.filter((t) => t.sport === 'soccer').map((team) => (
                                <option key={team.id} value={team.id}>
                                  {team.name} — {team.city} ({team.league})
                                </option>
                              ))}
                            </optgroup>
                          </>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-4 rounded-xl border border-white/12 bg-white/[0.04] p-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        {/* Color Swatches & Cross-Connector Adoption */}
                        <div className="flex flex-wrap items-center gap-2 mt-2">
                          <span className="text-[10px] text-white/40">Team Colors:</span>
                          <button
                            type="button"
                            onClick={() => onUpdateProfile({ accentColor: activeDropdownTeam.secondaryColor || activeDropdownTeam.primaryColor })}
                            title="Click to adopt team secondary color as active OS accent"
                            className="flex items-center gap-1.5 rounded-full border border-white/15 bg-white/5 px-2 py-0.5 text-[10px] text-white/80 hover:border-white/30 hover:bg-white/10 transition"
                          >
                            <span
                              className="h-3 w-3 rounded-full border border-white/20 shadow-xs shrink-0"
                              style={{ backgroundColor: activeDropdownTeam.secondaryColor }}
                            />
                            <span>Adopt {activeDropdownTeam.secondaryColor}</span>
                          </button>
                          <button
                            type="button"
                            onClick={() => onUpdateProfile({ accentColor: activeDropdownTeam.primaryColor })}
                            title="Click to adopt team primary color as active OS accent"
                            className="flex items-center gap-1.5 rounded-full border border-white/15 bg-white/5 px-2 py-0.5 text-[10px] text-white/80 hover:border-white/30 hover:bg-white/10 transition"
                          >
                            <span
                              className="h-3 w-3 rounded-full border border-white/20 shadow-xs shrink-0"
