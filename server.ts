...
const executiveTools = [
  {
    functionDeclarations: [
      ...
      },
      {
        ...
        description: 'Publishes an official announcement, special promotion, or event onto Google Search & Maps via Google Business Profile.',
        parameters: {
          type: Type.OBJECT,
          properties: {
            summary: { type: Type.STRING, description: 'Announcement body text or update copy' },
            call_to_action: { type: Type.STRING, description: 'Action button: BOOK, ORDER, LEARN_MORE, CALL' },
          },
          required: ['summary'],
        },
      },
      {
        ...
        description: 'Queries recent sales GMV and order fulfillment status from Shopify storefront.',
        parameters: {
          type: Type.OBJECT,
          properties: {
            status: { type: Type.STRING, description: 'Order status: open, unfulfilled, fulfilled, any' },
            limit: { type: Type.NUMBER, description: 'Max orders to inspect' },
          },
        },
      },
      {
        name: 'shopify_update_product',
        description: 'Updates price or inventory levels on a Shopify catalog product.',
        parameters: {
          type: Type.OBJECT,
          properties: {
            product_id: { type: Type.STRING, description: 'Shopify product or SKU ID' },
            price: { type: Type.STRING, description: 'New price in USD' },
            inventory_delta: { type: Type.NUMBER, description: 'Inventory delta count' },
          },
          required: ['product_id'],
        },
      },
      {
        ...
        description: 'Generates a secure Stripe hosted checkout link for one-off charges or retainers.',
        parameters: {
          type: Type.OBJECT,
          properties: {
            item_name: { type: Type.STRING, description: 'Product or retainer label' },
            amount_cents: { type: Type.NUMBER, description: 'Amount in cents (e.g. 250000 for $2,500.00)' },
          },
      ...
      },
      {
        ...
        description: 'Inspects recent Stripe payments, volume totals, and fee breakdown.',
        parameters: {
          type: Type.OBJECT,
          properties: {
            limit: { type: Type.NUMBER, description: 'Max charge records to retrieve' },
          },
        },
      },
      {
        name: 'gmail_draft_message',
        description: 'Prepares a polished response draft in Gmail workspace for user review before dispatch.',
        parameters: {
          type: Type.OBJECT,
          properties: {
      ...
      },
      {
        ...
        description: 'Queries email threads in Gmail inbox by sender or label query.',
        parameters: {
          type: Type.OBJECT,
          properties: {
            query: { type: Type.STRING, description: 'Search syntax (e.g. is:unread, from:client)' },
          },
        },
      },
      {
  ...
  for (const c of candidates) {
    ...
    for (const p of parts) {
      if (p.functionCall) {
        ...
        } else if (fc.name === 'gbp_post_update') {
          toolCall = {
            ...base,
            ...
            status: 'pending_confirmation',
            form_payload: {
              title: 'Google Business Announcement',
              items: [
                `Summary: "${String(args.summary || '')}"`,
                `Action Button: ${String(args.call_to_action || 'BOOK')}`,
              ],
              notes: 'Staged in Verification Tray. Click Approve to publish directly to Google Maps and Search.',
            },
          };
        } else if (fc.name === 'shopify_get_orders') {
          reply = 'I queried Shopify: Today gross volume is $18,420.00 across 42 orders. 6 orders unfulfilled and staged for courier.';
        } else if (fc.name === 'shopify_update_product') {
          toolCall = {
            ...base,
            target_app: 'Shopify Storefront & Orders',
            category: 'commerce_sales',
            action_name: 'Update Shopify Product Catalog',
            requires_user_confirmation: true,
            status: 'pending_confirmation',
            form_payload: {
              title: `Update Product ${String(args.product_id || '')}`,
              items: [
                `Product ID: ${String(args.product_id || '')}`,
                `Price: $${String(args.price || '')}`,
                `Inventory Delta: ${String(args.inventory_delta || 0)} units`,
              ],
              notes: 'Staged in Verification Tray. Click Approve to sync changes to Shopify.',
            },
          };
        } else if (fc.name === 'stripe_create_payment_link') {
          const amt = Number(args.amount_cents || 250000) / 100;
          toolCall = {
            ...base,
            ...
            status: 'pending_confirmation',
            form_payload: {
              title: `Payment Link: ${String(args.item_name || 'Retainer')} ($${amt.toFixed(2)})`,
              items: [
                `Item: ${String(args.item_name || 'Retainer')}`,
                `Amount: $${amt.toFixed(2)} USD`,
                'Methods: Card, ACH, Apple Pay enabled',
              ],
              notes: 'Staged in Verification Tray. Click Approve to activate payment link.',
            },
          };
        } else if (fc.name === 'stripe_get_charges') {
          reply = 'I checked Stripe: $24,650.00 gross volume across 18 transactions in the last 24h. 0 failed charges.';
        } else if (fc.name === 'gmail_draft_message') {
          toolCall = {
            ...base,
            target_app: 'Gmail & Google Workspace Mail',
            category: 'communication',
            action_name: 'Draft Email Reply',
            requires_user_confirmation: true,
            status: 'pending_confirmation',
            form_payload: {
              title: `Draft to ${String(args.to || '')}: "${String(args.subject || '')}"`,
              items: [
        ...
        } else if (fc.name === 'gmail_search_threads') {
          reply = 'I searched your Gmail inbox: 2 priority unread threads from Sarah Jenkins and Billing department.';
